...
pub mod solana_anchor {
        ...
        if last_number == 0 {
            for i in 0..nfts.len(){
                set_reward_rate(&mut ctx.accounts.pool_ledger, (last_number + i as u32) as usize, RewardRate{
                    reward_rate : 2000 + rasing_rate as u64 * i as u64,
                    mint : nfts[i],
                });
            }
        } else {
    ...
    pub fn deposit(
        ...
        if user_data.stake_amount + _amount > pool.deposit_limit {
...
pub const POOL_SIZE : usize = 32 + 32 + 32 + 32 + 32 + 32 + 8 + 8 + 8 + 8 + 8 + 8 + 8 + 1 + 1;
#[account]
pub struct Pool{
    pub owner : Pubkey,
